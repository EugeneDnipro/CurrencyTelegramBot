...
import com.goit.org.dto.MonobankService;
import com.goit.org.dto.NBUService;
...
public class BotAnswer {
    ...
    public void botAnswerUtils(String receivedMessage, SendMessage message, MessageSender sender) {
        switch (receivedMessage) {
            case "/start":
                ...
                if (file.exists()) {
                    preferences = CredentialsSavingUtil.credentialsGetFromFile();
                }
                if (preferences.get(message.getChatId()) == null) {
                    preferences.put(message.getChatId(), new UserPreferences());
                }
                if (!preferences.get(message.getChatId()).getTime().equals("вимкнено")) {
                    scheduleNotification(message, sender, preferences.get(message.getChatId()).getTime());
                }
                break;
            case "/help":
            ...
            case "18":
                ...
                CredentialsSavingUtil.credentialsSetToFile(preferences);
                break;
    ...
    private void scheduleNotification(SendMessage message, MessageSender sender, String hour) {
        ...
        MessageUtil.returnMenu(message, "Обраний час оповіщень: " + preferences.get(message.getChatId()).getTime());
    }
